...
import { deserializeMentions, serializeMentions } from "./mention-parser";
...
describe("serializeMentions", () => {
  ...
  describe("text with links", () => {
    ...
    it("should parse multiple URLs", () => {
      const text = "See https://example.com and https://test.com for details";
      const result = serializeMentions(text, [], mockUsers);
...
describe("deserializeMentions", () => {
  ...
  it("should use fallback for mention without username", () => {
    const comment: Comment = {
      ...
      taskId: "task-1",
      text: "Hey there",
      structuredComment: [{ text: "Hey " }, { type: "tag", user: { id: 789 } }],
      author: {
        id: 1,
        name: "Test",
        initials: "T",
